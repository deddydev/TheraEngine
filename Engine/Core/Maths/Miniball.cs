...
namespace TheraEngine.Maths
{
    public class Miniball
    {
        ...
        private FloatingPoint[] center, centerToAff, centerToPoint, lambdas;
    ...
    public class PointSetArray : IPointSet
    {
        private int _dimensions, _size;
        private FloatingPoint[] _values;

        public PointSetArray(int dimensions, int size)
        {
            _dimensions = dimensions;
            _size = size;
            _values = new FloatingPoint[size * dimensions];
        }

        public int Size =>  _size;
        public int Dimensions =>  _dimensions;

        public static PointSetArray FromVectors(Vec4[] points)
        {
            PointSetArray p = new PointSetArray(4, points.Length);
            for (int i = 0; i < points.Length; ++i)
                for (int j = 0; j < 4; ++j)
                    p._values[i * 4 + j] = points[i][j];
            return p;
        }
        public static PointSetArray FromVectors(Vec3[] points)
        {
            PointSetArray p = new PointSetArray(3, points.Length);
            for (int i = 0; i < points.Length; ++i)
                for (int j = 0; j < 3; ++j)
                    p._values[i * 3 + j] = points[i][j];
            return p;
        }
        public static PointSetArray FromVectors(Vec2[] points)
        {
            PointSetArray p = new PointSetArray(2, points.Length);
            for (int i = 0; i < points.Length; ++i)
                for (int j = 0; j < 2; ++j)
        ...
        {
            PointSetArray p = new PointSetArray(1, points.Length);
            for (int i = 0; i < points.Length; ++i)
        ...
        public FloatingPoint Coord(int i, int j)
        {
            //assert 0 <= i && i < n;
            //assert 0 <= j && j < d;
            return _values[i * _dimensions + j];
        }
    ...
    public class BitSet
    {
        ...
        public bool Get(int i)
            => _bits.IndexInRange(i) ? _bits[i] : false;
        public void Set(int i)
        {
            if (!_bits.IndexInRange(i))
                _bits.Resize(i + 1);
            _bits[i] = true;
        }
    ...
    {
        private IPointSet _points;
        private BitSet membership;
        private int dim;
        private int[] members;
        private FloatingPoint[][] Q, R;
        private FloatingPoint[] u, w;
        private int r;
    ...
    {
        private FloatingPoint _qrInconsistency;
        private FloatingPoint _minConvexCoefficient;
        private FloatingPoint _maxOverlength;
        private FloatingPoint _maxUnderlength;
        private int _iterations;
        private int _supportSize;
        ...
        public Quality(
            ...
            int supportSize)
        {
            _qrInconsistency = qrInconsistency;
            _minConvexCoefficient = minConvexCoefficient;
            _maxOverlength = maxOverlength;
            _maxUnderlength = maxUnderlength;
        ...
        /// </summary>
        public FloatingPoint QrInconsistency => _qrInconsistency;
        /// <summary>
        ...
        /// </summary>
        public FloatingPoint MinConvexCoefficient => _minConvexCoefficient;
        /// <summary>
        ...
        /// </summary>
        public FloatingPoint MaxOverlength => _maxOverlength;
        /// <summary>
        ...
        /// </summary>
        public FloatingPoint MaxUnderlength => _maxUnderlength;
        /// <summary>
        ...
        /// </summary>
        public int SupportSize => _supportSize;
