...
namespace System.ComponentModel
{
    ...
    public class TFile3rdPartyExt : Attribute
    {
        ...
        public bool HasExtension(string ext, bool allowWildcard)
        {
            if (string.IsNullOrWhiteSpace(ext))
                return false;
            //ext = ext.ToLowerInvariant();
            if (ext[0] == '.')
                ext = ext.Substring(0);
            return Extensions.Contains(ext, StringComparison.InvariantCultureIgnoreCase) || (allowWildcard && Extensions.Contains("*", StringComparison.InvariantCultureIgnoreCase));
        }
