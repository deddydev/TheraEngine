...
namespace TheraEngine.Rendering.Models
{
    ...
    public class PrimitiveManager : BaseRenderState, IPrimitiveManager
    {
        ...
        internal RenderProgram _vsFragProgram;
        ...
        private void SetSkinningUniforms(int programBindingId)
        {
            ...
            if (Engine.Settings.SkinOnGPU)
            {
                Engine.Renderer.ProgramUniform(programBindingId, Uniform.BoneMatricesName + "[0]", Matrix4.Identity);
                Engine.Renderer.ProgramUniform(programBindingId, Uniform.BoneMatricesITName + "[0]", Matrix4.Identity);
                ...
                foreach (int index in _modifiedBoneIndices)
                {
                    ...
                    ++remappedIndex;
                    Engine.Renderer.ProgramUniform(programBindingId, Uniform.BoneMatricesName + "[" + remappedIndex + "]", b.VertexMatrix);
                    Engine.Renderer.ProgramUniform(programBindingId, Uniform.BoneMatricesITName + "[" + remappedIndex + "]", b.NormalMatrix);
                }
        ...
        public unsafe void Render(Matrix4 modelMatrix, Matrix3 normalMatrix)
        {
            ...
            if (Engine.Settings.AllowShaderPipelines)
            {
                ...
                _pipeline.Set(EProgramStageMask.FragmentShaderBit, fragId = _material.Program.BindingId);
                //_pipeline.SetActive(_vertexProgram.BindingId);

            }
            else
            {
                vtxId = fragId = _vsFragProgram.BindingId;
                _vsFragProgram.Use();
            }
            ...
            int location = Uniform.GetLocation(vtxId, ECommonUniform.ModelMatrix);
            //if (location < 0)
            //    throw new Exception();
            Engine.Renderer.ProgramUniform(vtxId, location, modelMatrix);

            location = Uniform.GetLocation(vtxId, ECommonUniform.NormalMatrix);
            //if (location < 0)
            //    throw new Exception();
            Engine.Renderer.ProgramUniform(vtxId, location, normalMatrix);

            location = Uniform.GetLocation(vtxId, ECommonUniform.ViewMatrix);
            //if (location < 0)
            //    throw new Exception();
            Engine.Renderer.ProgramUniform(vtxId, location, AbstractRenderer.CurrentCamera.CameraToWorldMatrix);

            location = Uniform.GetLocation(vtxId, ECommonUniform.ProjMatrix);
            //if (location < 0)
            //    throw new Exception();
            Engine.Renderer.ProgramUniform(vtxId, location, AbstractRenderer.CurrentCamera.ProjectionMatrix);

            //Fragment shader
            AbstractRenderer.CurrentCamera.SetUniforms(fragId);
            if (Engine.Settings.ShadingStyle == ShadingStyle.Forward)
                Engine.Scene.Lights.SetUniforms(fragId);
            _material.SetUniforms(fragId);
        ...
        protected override void OnGenerated()
        {
            ...
            {
                if (_vsFragProgram == null)
                {
                    _vsFragProgram = new RenderProgram(_bufferInfo, Material.FragmentShaders[0], _vertexShader);
                    _vsFragProgram.Generate();
                }
            }
