...
namespace TheraEngine.ThirdParty.PMX
{
    public class PMXImporter
    {
        ...
        public PMXImporter(SkeletalModel model)
        {
            _model = model;
            _skeleton = _model?.SkeletonRef?.File;
        }
        public PMXImporter(SkeletalModel model, Skeleton skeleton)
        {
            _model = model;
            _skeleton = skeleton;
        }
                ...
                using (FileMap map = FileMap.FromStream(stream))
                {
                    ...
                    foreach (BaseSubMesh mesh in meshes)
                        foreach (var lod in mesh.LODs)
                        {
                            ...
                            if (mat != null)
                            {
                                string refPathAbs = mref.Path.Path;
                                    ...
                                    foreach (var tex in lod.MaterialRef.File.Textures)
                                    {
                                        if (tex is TexRef2D tex2D)
                                            foreach (var mip in tex2D.Mipmaps)
                                                if (!string.IsNullOrEmpty(mip.Path.Path) &&
                                                    mip.Path.Path.IsExistingDirectoryPath() == false)
                                                    texNames.Add(mip.Path.Path.MakeAbsolutePathRelativeTo(path));
                                    }
                            }
                        }
