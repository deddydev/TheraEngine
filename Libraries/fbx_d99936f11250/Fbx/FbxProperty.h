...
{				
	...
	* \nosubgrouping
	*/
	public ref class FbxPropertyManaged : FbxPropertyFlags, IFbxNativePointer
	{
		BASIC_CLASS_DECLARE(FbxPropertyManaged,FbxProperty);			
		GET_NATIVEPOINTER_DECLARE_BASE(FbxPropertyManaged,FbxProperty);	
		IS_SAME_AS_DECLARE_BASE();
	internal:
		FbxPropertyManaged(FbxProperty p)
		{
			_SetPointer(new FbxProperty(),true);
			*this->_FbxPropertyManaged = p;				
		}
	...
	public:
		...
		* \param pWasFound
		*/
		static FbxPropertyManaged^ Create(FbxPropertyManaged^ compoundProperty,
		...
		* \param pWasFound
		*/
		static FbxPropertyManaged^ Create(FbxObjectManaged^ obj,System::String^ name, FbxDataType^ dataType, System::String^ label,bool checkForDuplicate,bool %wasFound);
		static FbxPropertyManaged^ Create(FbxObjectManaged^ obj,System::String^ name, FbxDataType^ dataType, System::String^ label);
		...
		* \param pCheckForDuplicate
		*/
		static FbxPropertyManaged^ Create(FbxObjectManaged^ obj, FbxPropertyManaged^ fromProperty, bool checkForDuplicate);
		static FbxPropertyManaged^ Create(FbxObjectManaged^ obj, FbxPropertyManaged^ fromProperty);
		...
		*/
		DEFAULT_CONSTRUCTOR(FbxPropertyManaged,FbxProperty);

		/** Copy constructor for properties
		*/
		FbxPropertyManaged(FbxPropertyManaged^ p);

		/** Copy constructor for properties
		*/
		FbxPropertyManaged(FbxPropertyHandle^ p);			
	...
	public:
		...
		*/
		//@{
		void CopyFrom(FbxPropertyManaged^ p);
		virtual bool Equals(System::Object^ obj) override
		{
			FbxPropertyManaged^ o = dynamic_cast<FbxPropertyManaged^>(obj);
			if(o)
				return *_Ref() == *o->_Ref();
			return false;
		}
		...
		// return false;//*p1->pro != value;
		//}
		bool CompareValue(FbxPropertyManaged^ prop);
		//@}
		...
		*/
		bool CopyValue(FbxPropertyManaged^ prop);
	...
	public:
		...
		*/
		...
		FbxPropertyManaged^ GetParent();
		bool SetParent(FbxPropertyManaged^ other );
		FbxPropertyManaged^ GetChild();
		FbxPropertyManaged^ GetSibling();
		...
		*         an invalid KFbxProperty. See KFbxProperty::IsValid()
		*/
		FbxPropertyManaged^ Find (System::String^ name,bool caseSensitive);
		FbxPropertyManaged^ Find (System::String^ name,FbxDataType^ dataType, bool caseSensitive);
		/** Fullname find
		* \param pCaseSensitive
		* \param pName The name of the property as a \c NULL terminated string.
		* \return A valid KFbxProperty if the property was found, else
		*         an invalid KFbxProperty. See KFbxProperty::IsValid()
		*/
		FbxPropertyManaged^ FindHierarchical (System::String^ name,bool caseSensitive);
		/*
		...
		*/
		...
		public:
			FbxPropertyNameCache(FbxPropertyManaged^ prop);				
		};			         
		...
		*/
		...
		int  GetArraySize();
		FbxPropertyManaged^ GetArrayItem(int index);
		property FbxPropertyManaged^ default[int]
		{
			FbxPropertyManaged^ get(int index);				
		}
		//@}
	...
	public:
		// Properties
		bool ConnectSrcProperty(FbxPropertyManaged^ p);
		bool IsConnectedSrcProperty (FbxPropertyManaged^ p);
		bool DisconnectSrcProperty (FbxPropertyManaged^p);
		property int SrcPropertyCount
		{
			int get();
		}
		FbxPropertyManaged^ GetSrcProperty (int index);
		FbxPropertyManaged^ FindSrcProperty(System::String^ name,int startIndex);

		bool ConnectDstProperty(FbxPropertyManaged^ p);
		bool IsConnectedDstProperty  (FbxPropertyManaged^ p);
		bool DisconnectDstProperty(FbxPropertyManaged^ p);
		property int DstPropertyCount
		{
			int get();
		}

		FbxPropertyManaged^ GetDstProperty (int index);
		FbxPropertyManaged^ FindDstProperty(System::String^ name,int startIndex);
	...
	public:
		...
		{
			Unidentified = FbxProperty::eUNIDENTIFIED,
			Bool= FbxProperty::eBOOL,
			Real = FbxProperty::eREAL,
			Color = FbxProperty::eCOLOR,
			Integer = FbxProperty::eINTEGER,
			Vector = FbxProperty::eVECTOR,
			List = FbxProperty::eLIST,
			Matrix = FbxProperty::eMATRIX
		};
		...
		static FbxDataType^ UserPropertyTypeToDataType(FbxPropertyManaged::UserPropertyType type);
		static FbxPropertyManaged::UserPropertyType DataTypeToUserPropertyType(FbxDataType^ dataType);
	...
	* \nosubgrouping
	*/
	public ref class FbxConnectEventManaged : IFbxNativePointer
	{
		BASIC_CLASS_DECLARE(FbxConnectEventManaged,FbxConnectEvent);
		INATIVEPOINTER_DECLARE(FbxConnectEventManaged,FbxConnectEvent)		
	protected:
		FbxPropertyManaged^ _Source;
		FbxPropertyManaged^ _Destination;

	public:
		FbxConnectEventManaged(FbxConnectEventType type,
		...
		property FbxPropertyManaged^ Source
		{
			FbxPropertyManaged^ get();
		}			    
		property FbxPropertyManaged^ Destination
		{
			FbxPropertyManaged^ get();
		}
