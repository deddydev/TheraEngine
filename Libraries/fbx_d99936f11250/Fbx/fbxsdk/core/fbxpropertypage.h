...
inline FbxPropertyInfo* Get(const FbxPropertyInfo*  ){ return mInfo; }
...
inline FbxPropertyValue* Get(const FbxPropertyValue*  ){ return mValue; }
...
inline FbxPropertyConnect* Get(const FbxPropertyConnect*  ){ return mConnect; }
...
inline FbxPropertyFlags* Get(const FbxPropertyFlags*  ){ return &mFlags; }
...
inline bool Reparent( FbxInt  , FbxInt   )
{
return false;
}
...
{
if( lReferencePage == this ) return FbxPropertyFlags::eOverride;
else if( !lReferencePage->mInstanceOf ) return FbxPropertyFlags::eInherit;
...
return FbxPropertyFlags::eOverride;
else
lFoundOverride = true;
}
...
if( lFoundIn != this ) return true;
lPropertyFlags->UnsetMask( pFlags );
if( lPropertyFlags->GetMask() == 0 )
ChangePropertyItemState( FBX_TYPE(FbxPropertyFlags), pId, FbxPropertyFlags::eInherit );
return true;
}
return false;
